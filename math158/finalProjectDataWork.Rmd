...
```{r step function}
  step(lmod1)
```
giving us the same linear model. now try backward elimination for followerCount,
```{r backward elimination}
  lmodRed <- lm(likeCount ~ tweetId + replyCount + retweetCount + sentiments + accessPoint + textLength + mentions, data = dfTrain)
  summary(lmodRed)
  anova(lmod1, lmodRed)
```
...
```{r checking normality assumptions}
  shapiro.test(sample(resid(lmod), 5000, replace = FALSE, prob = NULL))
```
...
```{r lts method}
  ...
  predLmod <- predict(lmod, dfPredict)
  predLTS <- predict(ltsmod, dfPredict)
  print("Original Model:")
  rmse(predLmod, dfPredict$likeCount)
  print("LTS Model:")
